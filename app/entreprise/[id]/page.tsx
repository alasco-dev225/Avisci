...
export default function EntreprisePage() {
  ...
  async function signalerAvis(avisId: string) {
    ...
    alert(error ? `Erreur: ${error.message}` : 'Merci, le signalement a été envoyé.')
  }
              ...
              <form onSubmit={soumettreAvis} className="space-y-4">
                ...
                <div className="border rounded-lg p-3">
                  ...
                  <div className="flex gap-2 items-center flex-wrap">
                    ...
                    {audioBlob && <audio controls src={URL.createObjectURL(audioBlob)} className="h-10" />}
                  </div>
                </div>
            ...
            {avis.map((a) => (
              <div key={a.id} className="bg-white rounded-xl border p-4">
                <div className="flex items-start justify-between gap-2">
                  ...
                  <button onClick={() => signalerAvis(a.id)} className="text-xs text-red-500 hover:underline">Signaler</button>
                </div>
                ...
                {isOwner && (
                  <form
                    ...
                    onSubmit={(e) => {
                      e.preventDefault()
                      const value = (new FormData(e.currentTarget).get('reponse') as string) || ''
                      repondreAvis(a.id, value)
                    }}
                  >
                    <textarea
                      name="reponse"
                      rows={2}
                      defaultValue={reponses[a.id]?.contenu || ''}
